...
void lcd_clear_layer2_memory(void) { // 清空第二层内存
  // printf("Clearing second layer memory...\n"); // 打印调试信息（已注释）
  
  uint32_t buffer_size = lcd_params.hres * lcd_params.vres * 2; // RGB565 is 2 bytes // 计算缓冲区大小
  ...
  for (uint32_t i = 0; i < buffer_size / 2; i++) { // 遍历每个像素
    layer2_buffer[i] = 0x0000; // 清空为黑色
  }
  
  // printf("Second layer memory clearing completed, buffer size: %lu bytes\n", buffer_size); // 打印完成信息（已注释）
}
...
// 设置Layer1背景图片路径
void lcd_cover_background_set_layer1_path(const char* path) { // 设置Layer1背景图片路径
  if (path && strlen(path) < sizeof(g_layer1_background_path)) { // 路径有效且长度合适
    strncpy(g_layer1_background_path, path, sizeof(g_layer1_background_path) - 1); // 拷贝路径
    g_layer1_background_path[sizeof(g_layer1_background_path) - 1] = '\0'; // 确保结尾为0
    printf("CoverBackground: Layer1 background path set to: %s\n", g_layer1_background_path); // 打印设置信息
  }
}
...
void lcd_cover_background_sync_layer1_background(void) { // 同步Layer1背景为指定图片
  if (!g_layer2_initialized) { // 如果Layer2未初始化
    return; // 直接返回
  }
  ...
  if (strlen(g_layer1_background_path) > 0) { // 路径有效
    printf("CoverBackground: Loading background image to Layer1: %s\n", g_layer1_background_path); // 打印加载信息
    
    // 使用专用的JPEG输出缓冲区
    uint32_t jpeg_output_address = FMC_SDRAM_JPEG_OUTPUT_DATA_BUFFER_ADDRESS; // JPEG输出缓冲区地址
    
    // 初始化JPEG解码器
    jpeg_init(); // 初始化JPEG解码器
    
    // 解码JPEG文件到临时缓冲区
    int decode_result = jped_decode(g_layer1_background_path, jpeg_output_address); // 解码JPEG
    
    if (decode_result == 0) { // 解码成功
      // 获取解码后的图片信息
      uint32_t width, height, subsampling; // 图片宽高和采样方式
      jpeg_decode_info(&width, &height, &subsampling); // 获取图片信息
      
      printf("CoverBackground: JPEG decoded for Layer1: %lux%lu, subsampling: %lu\n", width, height, subsampling); // 打印图片信息
      
      // 计算需要复制的像素数量
      uint32_t copy_width = (width > lcd_params.hres) ? lcd_params.hres : width; // 取最小宽度
      uint32_t copy_height = (height > lcd_params.vres) ? lcd_params.vres : height; // 取最小高度
      
      // 使用DMA2D复制解码后的图片到Layer1
      if (subsampling == JPEG_420_SUBSAMPLING || subsampling == JPEG_422_SUBSAMPLING || subsampling == JPEG_444_SUBSAMPLING) { // YCbCr采样
        dma2d_copy_ycbcr_to_rgb((uint32_t*)jpeg_output_address, 
                                (uint32_t*)layer1_addr,
                                copy_width, copy_height, subsampling); // YCbCr转RGB并拷贝
      } else {
        dma2d_copy_buffer((uint32_t*)jpeg_output_address, 
                          (uint32_t*)layer1_addr,
                          0, 0, copy_width, copy_height); // 直接拷贝
      }
      
      printf("CoverBackground: Layer1 background synchronized successfully\n"); // 打印成功信息
    } else {
      printf("CoverBackground: Failed to decode background image for Layer1: %s, error: %d\n", g_layer1_background_path, decode_result); // 打印失败信息
    }
  }
}
...
void lcd_cover_background_move_to_y(int16_t y_position) {
  ...
  // 如果在动画期间且状态栏应该不透明，需要重新恢复状态栏数据
  // 因为ltdc_layer_config可能重新配置了整个layer
  if (g_animation_in_progress && !g_statusbar_transparent && g_statusbar_backup_valid && g_statusbar_backup_buffer != NULL) {
    printf("CoverBackground: Restoring statusbar data after layer reconfiguration during animation\n");
    uint16_t* layer2_buffer = (uint16_t*)LAYER2_MEMORY_BASE;
