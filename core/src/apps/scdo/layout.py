...
def require_confirm_fee(
    ...
    return confirm_total_ethereum(
        ctx,
        format_scdo_amount(value, token),
        None,
        format_scdo_amount(fee_limit, None),
        from_address,
        to_address,
        format_scdo_amount(value + fee_limit, None) if token is None else None,
        raw_data=raw_data,
    )
...
def require_show_overview(
    ...
    return should_show_details(
        ...
        title=_(i18n_keys.TITLE__SEND_MULTILINE).format(
            strip_amount(format_scdo_amount(value, token))[0]
        ),
...
def format_scdo_amount(value: int, token: tokens.TokenInfo | None) -> str:
    if token:
        suffix = token.symbol
