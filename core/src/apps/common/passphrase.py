...
async def _request_from_user(ctx: wire.Context) -> str:  # 从用户请求密码短语的内部异步函数
    ...
    if storage.device.get_passphrase_always_on_device() or issubclass(  # 如果设置为始终在设备上输入密码短语，或者是特定上下文类型, 就是防止调用软件端
...
async def _request_on_host(ctx: wire.Context) -> str:  # 在主机上请求密码短语的内部异步函数  必请求 软件端
