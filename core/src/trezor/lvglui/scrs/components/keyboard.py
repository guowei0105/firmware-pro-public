...
class PassphraseKeyboard(lv.btnmatrix):  # 密码短语键盘类，继承自按钮矩阵
    def __init__(self, parent, max_len, min_len=0) -> None:  # 初始化方法，接收父对象、最大长度和最小长度参数
        ...
        # line1  # 第一行按钮控制设置
        self.ctrl_map = [lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.POPOVER] * 10  # 设置前10个按钮为不重复且有弹出效果
        # line2  # 第二行按钮控制设置
        self.ctrl_map.extend([3 | lv.btnmatrix.CTRL.NO_REPEAT])  # 添加宽度为3的不重复按钮
        self.ctrl_map.extend(  # 扩展控制映射
            [7 | lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.POPOVER] * 9  # 9个宽度为7的不重复弹出按钮
        )
        self.ctrl_map.extend([3 | lv.btnmatrix.CTRL.NO_REPEAT])  # 添加宽度为3的不重复按钮
        # line3  # 第三行按钮控制设置
        self.ctrl_map.extend([2 | lv.btnmatrix.CTRL.NO_REPEAT])  # 添加宽度为2的不重复按钮
        self.ctrl_map.extend(  # 扩展控制映射
            [7 | lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.CLICK_TRIG]  # 宽度为7的不重复点击触发按钮
        )
        self.ctrl_map.extend(  # 扩展控制映射
            [5 | lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.POPOVER] * 7  # 7个宽度为5的不重复弹出按钮
        )
        self.ctrl_map.extend([4 | lv.btnmatrix.CTRL.NO_REPEAT])  # 添加宽度为4的不重复按钮
        # line4  # 第四行按钮控制设置
        self.ctrl_map.extend([3])  # 添加宽度为3的按钮
        self.ctrl_map.extend(  # 扩展控制映射
            [2 | lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.CLICK_TRIG]  # 宽度为2的不重复点击触发按钮
        )
        self.ctrl_map.extend(  # 扩展控制映射
            [7 | lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.CLICK_TRIG]  # 宽度为7的不重复点击触发按钮
        )
        self.ctrl_map.extend(  # 扩展控制映射
            [3 | lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.CLICK_TRIG]  # 宽度为3的不重复点击触发按钮
        )
    ...
    def update_count_tips(self):  # 更新计数提示方法
        self.input_count_tips.set_text(f"{len(self.ta.get_text())}/50")  # 设置计数提示文本，显示当前长度/50
    ...
    def update_ok_button_state(self):  # 更新OK按钮状态方法
        ...
        if current_len >= self.min_len:  # 如果当前长度大于等于最小长度
            ...
            # 确保有点击触发控制  # 注释：设置点击触发
            self.set_btn_ctrl(34, lv.btnmatrix.CTRL.NO_REPEAT | lv.btnmatrix.CTRL.CLICK_TRIG)  # 设置按钮为不重复且点击触发
        else:  # 否则
            # 禁用OK按钮  # 注释：禁用按钮
